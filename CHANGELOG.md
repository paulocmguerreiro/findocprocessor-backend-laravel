...
- **Issue #114** — Reorganiza a estrutura de pastas da feature `Documento` aplicando a regra de granularidade pasta-por-Action (`02-shared/estrutura-subpastas-features.md`, decidida em WRN-037); refactor estrutural puro, sem alteração de comportamento
  ...
  - Nova `Operacoes/TransicoesEstado/` consolida os 8 atalhos de transição de estado (5 `MarcarAnalise*` + `MarcarErro` + `MarcarPerigoso` + `TransicionarProcessado`) — nome escolhido pela intenção (transição), não pelo mecanismo ("marcar"); `Atribuicao/` mantida com as 2 `Reivindicar*`, não dissolvida
  - Só localização/namespace muda (25 renomeações de código + 1 de teste) — mesma bateria de testes verde (100% cobertura + type coverage, Larastan 9)
