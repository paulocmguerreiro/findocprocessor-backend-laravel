...
### Tarefa 1 — Config: modelo por camada

- Ficheiros a criar/alterar: `config/extracao.php`
- O que implementar: acrescentar `'local' => ['modelo' => env('LLM_LOCAL_MODEL')]` e
  `'cloud' => ['modelo' => env('LLM_CLOUD_MODEL')]` (mantendo `camada_local_activa`/
  `camada_cloud_activa` inalterados). Comentário inline a explicar que estas chaves evitam
  `env()` fora de ficheiro de config no `ClienteExtracaoIAPrism`.
- Testes associados: `tests/Unit/Config/ExtracaoConfigTest.php` (já existe — acrescentar
  asserções para `extracao.local.modelo`/`extracao.cloud.modelo` reflectirem `LLM_LOCAL_MODEL`/
  `LLM_CLOUD_MODEL`).
- Commit: `feat(extracao): expõe modelo LLM por camada em config/extracao.php`
...
- O que implementar (`final class implements ClienteIA`):
  1. Mapa `CamadaIA → array{provider: Provider, modelo: string}` a partir de `config('extracao.local.modelo')`/`config('extracao.cloud.modelo')` — `Provider::Ollama` para `Local`, `Provider::OpenAI` para `Cloud`.
  ...
  5. Chamada: `Prism::structured()->using($provider, $modelo)->withSchema($schema)->withSystemPrompt($systemPrompt)->withPrompt($mensagemComNonce)->asStructured()`, dentro de `try/catch (\Throwable)` — qualquer excepção → `ResultadoExtracaoIA::falhaTecnica($excepcao->getMessage())` (RF-07.4, RNF-06: nunca incluir `$textoExtraido` na mensagem capturada).
...
- Mapeamento `CamadaIA::Local → Provider::Ollama` / `CamadaIA::Cloud → Provider::OpenAI` é fixo no
  código (Brief) — se `config/prism.php` viesse a registar outro provider para uma das camadas, o
  mapa desta tarefa teria de mudar também; aceite como acoplamento conhecido, não uma abstracção
  extra nesta issue.
