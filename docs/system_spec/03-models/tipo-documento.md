...
| `created_at` / `updated_at` | `timestamp` | Sim | — | `timestamps()`; **sem `deleted_at`** (hard-delete deliberado) |
...
- Sem `deleted_at` — a tabela não é referenciada por FK de nenhuma tabela filha (coerente com `00-convencoes-models.md`).
- `cascadeOnUpdate()` — sem esta cascade, um `UPDATE` à PK da categoria falharia por violação de FK; prepara para uma futura reconciliação/agregação de bases de dados que precise de remapear UUIDs.
...
`final readonly class`, estrutura idêntica (update completo/PUT), com `fromRequest()`. Construtor valida:
...
- RN-04: `posicao_empresa_mae` determina se a entidade com `e_empresa_aplicacao = true` aparece como `Fornecedor` ou `Cliente` — regra de leitura pelo pipeline de extracção (`RegraReconciliarEntidadesDocumento`, ver `01-features/documento-pipeline.md`); sem lógica de validação nesta camada.
...
- **Actions/Controller/FormRequests/rotas** documentados em `01-features/tipo-documento.md` — fora do âmbito deste ficheiro (Model/Migration/Factory).
