...
| `deleted_at` | `timestamp` nullable | `Carbon\|null` | SoftDeletes |
...
- `SoftDeletes` — eliminação lógica (`deleted_at`); ver `02-shared/soft-delete.md`
- `FiltravelPorEstadoRegisto` — scope transversal `filtrarPorEstadoRegisto(FiltroEstadoRegisto)`
...
> **Referência por FKs filhas (`restrictOnDelete` + `cascadeOnUpdate`):** `documentos.id_responsavel` e `etapas_documento.id_utilizador` apontam para `users` com `restrictOnDelete`. Um utilizador referenciado não pode ser hard-deleted — `EliminarUtilizadorAction` cai no fallback para soft delete, preservando a autoria. `cascadeOnUpdate()` adicionado em 2026-07-14 (migration `add_cascade_on_update_to_domain_fks`) — sem esta cascade, um `UPDATE` ao `id` (bigint incremental) de `users` falharia por violação de FK; prepara para uma futura agregação de bases de dados onde os IDs incrementais de `users` precisem de ser remapeados.
...
`#[UsePolicy(UtilizadorPolicy::class)]` — a `UtilizadorPolicy` expõe `atribuirRole`, `viewAny`, `view` (com auto-acesso), `create`, `update`, `delete`. Detalhe em `01-features/utilizador.md` e `04-infra/autorizacao.md`.
