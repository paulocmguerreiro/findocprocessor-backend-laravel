...
> **Referência por FKs filhas (`restrictOnDelete` + `cascadeOnUpdate`):** `documentos.id_responsavel` e `etapas_documento.id_utilizador` apontam para `users` com `restrictOnDelete` (#68). Um utilizador referenciado não pode ser hard-deleted — `EliminarUtilizadorAction` cai no fallback para soft delete, preservando a autoria. `cascadeOnUpdate()` adicionado em 2026-07-14 (migration `add_cascade_on_update_to_domain_fks`) — sem esta cascade, um `UPDATE` ao `id` (bigint incremental) de `users` falharia por violação de FK; prepara para uma futura agregação de bases de dados onde os IDs incrementais de `users` precisem de ser remapeados.
