...
> **Referência por FKs filhas (`restrictOnDelete` + `cascadeOnUpdate`):** `documentos.id_responsavel` e `etapas_documento.id_utilizador` apontam para `users` com `restrictOnDelete`. Um utilizador referenciado não pode ser hard-deleted — `EliminarUtilizadorAction` cai no fallback para soft delete, preservando a autoria. `cascadeOnUpdate()` — sem esta cascade, um `UPDATE` ao `id` (bigint incremental) de `users` falharia por violação de FK; prepara para uma futura agregação de bases de dados onde os IDs incrementais de `users` precisem de ser remapeados.
