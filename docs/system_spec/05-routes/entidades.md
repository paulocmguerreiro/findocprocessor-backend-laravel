...
**SoftDeletes (#71):** `show`/`update`/`destroy` e `restaurar` usam `->withTrashed()` para o binding incluir registos soft-deleted — sem isto, restaurar/ver um inactivo daria 404. `index` mantém-se activo-por-omissão (filtra via `?estado=`). Padrão completo em `02-shared/soft-delete.md`.
...
| `estado` | string | `somente_ativos` | valores de `FiltroEstadoRegisto` (`todos`, `somente_ativos`, `somente_inativos`) | Filtro de estado SoftDelete (#71) |
