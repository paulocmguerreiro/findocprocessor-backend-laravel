...
`nome` `Rule::unique` — `tipos_documento.nome` tem índice único na BD; sem esta regra, um duplicado geraria `QueryException` (500) em vez de 422.

### `withValidator()` — validação cross-field RN-02
